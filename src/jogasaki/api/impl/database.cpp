...
auth::action_kind from(proto::metadata::storage::TableActionKind kind) {
    using proto::metadata::storage::TableActionKind;
    using auth::action_kind;
    switch(kind) {
...
status database::recover_index_metadata(
    ...
    for(auto&& n : keys) {
        ...
        if(primary_only) {
            ...
            from_authorization_list(idef.table_definition(), s->authorized_actions());
        }
