...
public:
    ...
    producer_flow(exchange::step* downstream, model::step* step, channel* ch) : downstream_(downstream), step_(step), channel_(ch) {}
    takatori::util::sequence_view<std::unique_ptr<model::task>> create_tasks() override {
        ...
        tasks_.emplace_back(std::make_unique<producer_task>(channel_, step_, &writer));
