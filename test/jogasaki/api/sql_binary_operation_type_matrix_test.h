...
class sql_binary_operation_type_matrix_test :
    public ::testing::Test,
...
public:
    ...
    void test_binary_operation_with_type(
        std::string_view expression,
        std::string_view l_type_name,
        std::string_view r_type_name,
        std::string_view values,
        std::optional<runtime_t<ResultKind>> exp,
        meta::field_type type = meta::create_field_type<ResultKind>()
    ) {
        execute_statement("create table t (c0 "+std::string{l_type_name}+", c1 "+std::string{r_type_name}+")");
        execute_statement("insert into t values "+std::string{values}+"");
        {
            std::vector<mock::basic_record> result{};
            execute_query("SELECT "+std::string{expression}+" FROM t", result);
