...
data::any substring(evaluator_context&, sequence_view<data::any> args) {
    ...
    auto extract_substring = [&](std::string_view view, auto type_tag) -> data::any {
        constexpr bool is_character_type = std::is_same_v<std::decay_t<decltype(type_tag)>,
            std::in_place_type_t<runtime_t<kind::character>>>;
        ...
        const auto start_byte =
            impl::get_start_index_byte(view, zero_based_start, is_character_type);
