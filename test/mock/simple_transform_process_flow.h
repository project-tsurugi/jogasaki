...
public:
    ...
    ~simple_transform_process_flow() = default;
    simple_transform_process_flow(exchange::step* downstream, model::step* step, channel* ch) : downstream_(downstream), step_(step), channel_(ch) {}
...
//        auto& writer = sinks[0].acquire_writer();
        tasks_.emplace_back(std::make_unique<simple_transform_process_task>(channel_, step_));
