 ...
 * re-insertability, and generated identity values after CONTINUE IDENTITY.
 * Internal sequence metadata is not inspected.
 *
 ...
 *     the sequence position is preserved (CONTINUE IDENTITY, the only
 *     supported behavior).
 ...
 *   - RESTART IDENTITY is not supported and causes a compile error.
 */
...
// CONTINUE IDENTITY (the default, and the only supported behavior) keeps it
// at N, so the next INSERT gets N+1.
...
TEST_F(sql_truncate_table_test, restart_identity_is_compile_error) {
    execute_statement("CREATE TABLE t (c0 INT PRIMARY KEY, c1 INT GENERATED ALWAYS AS IDENTITY)");
    test_stmt_err("TRUNCATE TABLE t RESTART IDENTITY", error_code::unsupported_compiler_feature_exception);
}
