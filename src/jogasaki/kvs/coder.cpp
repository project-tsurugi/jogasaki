...
void encode(
    ...
    switch(type.kind()) {
        ...
        case kind::character: dest.write<runtime_t<kind::character>>(src.get_value<runtime_t<kind::character>>(offset), odr, vi.add_padding(), vi.length()); break;
        default:
            fail();
    }
}
...
void encode(
    ...
    switch(type.kind()) {
        ...
        case kind::character: dest.write<runtime_t<kind::character>>(src.to<runtime_t<kind::character>>(), odr, vi.add_padding(), vi.length()); break;
        default:
            fail();
    }
}
