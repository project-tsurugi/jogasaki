 ...
 *          same query returns the correct result (1 row) when the TVF does not yield.
 *          the blocking variant is a regression test for the bug: with the fix, yield
 *          is correctly propagated from apply through aggregate_group::finish(), and the
 *          query returns 1 row as expected.
 */
 ...
 *          aggregate_group::finish().  the fix moves the empty-group handling from
 *          aggregate_group::finish() to a process_group(member_kind::empty) call in
 *          take_group::operator()(), so that yield is propagated correctly and the
 *          query returns the expected 1 row.
 */
