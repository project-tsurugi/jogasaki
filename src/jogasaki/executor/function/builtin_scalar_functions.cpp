...
enum class EncodingType { ASCII_1BYTE, UTF8_2BYTE, UTF8_3BYTE, UTF8_4BYTE, INVALID };
//
...
bool isContinuation_byte(unsigned char c) { return (c & 0xC0U) == 0x80U; }
EncodingType detectNextEncoding(std::string_view view, const size_t offset) {
    if (view.empty()) return EncodingType::INVALID;
    if (offset >= view.size()) return EncodingType::INVALID;
    const auto offset_2nd = offset + 1;
    const auto offset_3rd = offset + 2;
    auto first            = static_cast<unsigned char>(view[offset]);
    if (first <= 0x7FU) { return EncodingType::ASCII_1BYTE; }
    if (first >= 0xC2U && first <= 0xDFU) {
        return (view.size() >= 2 && isContinuation_byte(view[offset_2nd]))
                   ? EncodingType::UTF8_2BYTE
                   : EncodingType::INVALID;
    }
    if (first >= 0xE0U && first <= 0xEFU) {
        return (view.size() >= 3 && isContinuation_byte(view[offset_2nd]) &&
                   isContinuation_byte(view[offset_3rd]))
                   ? EncodingType::UTF8_3BYTE
                   : EncodingType::INVALID;
    }
    if (first >= 0xF0U && first <= 0xF4U) {
        const auto offset_4th = offset + 3;
        return (view.size() >= 4 && isContinuation_byte(view[offset_2nd]) &&
                   isContinuation_byte(view[offset_3rd]) && isContinuation_byte(view[offset_4th]))
                   ? EncodingType::UTF8_4BYTE
                   : EncodingType::INVALID;
    }
    return EncodingType::INVALID;
}
size_t get_byte(EncodingType e) {
    switch (e) {
        case EncodingType::ASCII_1BYTE: return 1;
        case EncodingType::UTF8_2BYTE: return 2;
        case EncodingType::UTF8_3BYTE: return 3;
        case EncodingType::UTF8_4BYTE: return 4;
        case EncodingType::INVALID: return 1; // or 0
    }
    return 0;
}
size_t getStartIndexByte(std::string_view view, const int64_t zero_based_start) {
    size_t tmp_byte = 0;
    size_t offset   = 0;
    for (int64_t i = 0; i < zero_based_start; ++i) {
        tmp_byte = get_byte(detectNextEncoding(view, offset));
        offset += tmp_byte;
    }
    // offset is sum of tmp_bytes
    return offset;
}
int64_t getCharLen(std::string_view view) {
    int64_t count   = 0;
    size_t offset   = 0;
    size_t tmp_byte = 0;
...
size_t getsizeByte(std::string_view view, const size_t start_byte, const size_t letter_count) {
    size_t tmp_byte = 0;
...
data::any substring(evaluator_context&, sequence_view<data::any> args) {
    ...
    auto extract_substring = [&](std::string_view view, auto type_tag) -> data::any {
        const auto view_size = view.size();
        if (view_size == 0 || zero_based_start < 0 ||
            static_cast<std::size_t>(zero_based_start) >= view_size) {
            return {};
        }
        const auto start_byte = getStartIndexByte(view, zero_based_start);
        ...
        if (casted_length) {
            ...
            if (casted_length_value > 0) {
                const auto substr_length_byte = getsizeByte(view, start_byte, casted_length_value);
    ...
    if (src.type_index() == data::any::index<accessor::binary>) {
        ...
        return extract_substring(
            static_cast<std::string_view>(bin), std::in_place_type<runtime_t<kind::octet>>);
    }
