...
| `0xfa` | `clob` | `#fa, p=b(8), s=b(8), t=b(8)` | `p` と `s` と `t` を big-endian の 64 ビット符号なし整数とみなし、プロバイダを `p` ( `1` の場合は データストア, `2` の場合は SQL 実行エンジン), BLOB 参照を `s`, BLOB 参照タグを `t` とする CLOB |
| `0xfb` | `blob` | `#fb, p=b(8), s=b(8), t=b(8)` | `p` と `s` と `t` を big-endian の 64 ビット符号なし整数とみなし、プロバイダを `p` ( `1` の場合は データストア, `2` の場合は SQL 実行エンジン), BLOB 参照を `s`, BLOB 参照タグを `t` とする BLOB |
| `0xfc` | _reserved_ | `#fc`
| `0xfd` | _reserved_ | `#fd`
