...
// ─── custom start value and increment with RESTART ────────────────────────────
//
// RESTART IDENTITY must reset to the start value and resume using the
// original increment step defined at table creation.

TEST_F(sql_truncate_table_test, generated_always_identity_custom_start_increment_restart) {
    execute_statement("CREATE TABLE t (c0 INT PRIMARY KEY, c1 INT GENERATED ALWAYS AS IDENTITY (START WITH 10 INCREMENT BY 5))");
    execute_statement("INSERT INTO t (c0) VALUES (1)");
    execute_statement("INSERT INTO t (c0) VALUES (2)");
    execute_statement("INSERT INTO t (c0) VALUES (3)");
    {
        ...
        ASSERT_EQ(3, result.size());
        EXPECT_EQ((create_nullable_record<kind::int4>(10)), result[0]);
        EXPECT_EQ((create_nullable_record<kind::int4>(15)), result[1]);
        EXPECT_EQ((create_nullable_record<kind::int4>(20)), result[2]);
    }
    execute_statement("TRUNCATE TABLE t RESTART IDENTITY");
    execute_statement("INSERT INTO t (c0) VALUES (10)");
    execute_statement("INSERT INTO t (c0) VALUES (11)");
    execute_statement("INSERT INTO t (c0) VALUES (12)");
    {
        std::vector<mock::basic_record> result{};
        execute_query("SELECT c1 FROM t ORDER BY c0", result);
        ASSERT_EQ(3, result.size());
...
// ─── RESTART IDENTITY on a sequence that cannot be restarted ─────────────────
//
// RESTART IDENTITY resets the sequence by storing the value just before the start value, that is
// `start value - increment`, so that the value generated next becomes the start value. When that
// value is out of the range of bigint, the sequence cannot be restarted and the statement results
// in an unsupported error. Note this never happens for the sequence generated for an implicit
// surrogate PK, whose start value is 0 and increment is 1.

TEST_F(sql_truncate_table_test, restart_identity_unsupported_generated_always) {
    execute_statement("CREATE TABLE t (c0 INT PRIMARY KEY, "
                      "c1 BIGINT GENERATED ALWAYS AS IDENTITY "
                      "(MINVALUE -9223372036854775808 START WITH -9223372036854775807 INCREMENT BY 2))");
    execute_statement("INSERT INTO t (c0) VALUES (1)");
    test_stmt_err("TRUNCATE TABLE t RESTART IDENTITY", error_code::unsupported_runtime_feature_exception);
    {
        // the statement failed before deleting the rows
        std::vector<mock::basic_record> result{};
        execute_query("SELECT c0 FROM t", result);
        ASSERT_EQ(1, result.size());
        EXPECT_EQ((create_nullable_record<kind::int4>(1)), result[0]);
    }
    // CONTINUE IDENTITY is not affected because it doesn't touch the sequence value
    execute_statement("TRUNCATE TABLE t CONTINUE IDENTITY");
    {
        std::vector<mock::basic_record> result{};
        execute_query("SELECT c0 FROM t", result);
    ...
    // the sequence keeps working after the failed statement
    execute_statement("INSERT INTO t (c0) VALUES (2)");
    {
        std::vector<mock::basic_record> result{};
        execute_query("SELECT c1 FROM t WHERE c0 = 2", result);
        ASSERT_EQ(1, result.size());
        EXPECT_EQ((create_nullable_record<kind::int8>(-9223372036854775805L)), result[0]);
    }
}

TEST_F(sql_truncate_table_test, restart_identity_unsupported_generated_by_default) {
    execute_statement("CREATE TABLE t (c0 INT PRIMARY KEY, "
                      "c1 BIGINT GENERATED BY DEFAULT AS IDENTITY "
                      "(MINVALUE -9223372036854775808 START WITH -9223372036854775808 INCREMENT BY 1))");
    execute_statement("INSERT INTO t (c0) VALUES (1)");
    test_stmt_err("TRUNCATE TABLE t RESTART IDENTITY", error_code::unsupported_runtime_feature_exception);
    {
        std::vector<mock::basic_record> result{};
    ...
    // TRUNCATE TABLE without the option (CONTINUE IDENTITY) succeeds
    execute_statement("TRUNCATE TABLE t");
    {
        std::vector<mock::basic_record> result{};
        execute_query("SELECT c0 FROM t", result);
...
TEST_F(sql_truncate_table_test, restart_identity_unsupported_aborts_tx) {
    // the unsupported error aborts the transaction that issued TRUNCATE TABLE, so the transaction
    ...
    execute_statement("INSERT INTO t (c0) VALUES (1)");
    {
        ...
        test_stmt_err("TRUNCATE TABLE t RESTART IDENTITY", *tx, error_code::unsupported_runtime_feature_exception);
        // verify tx abort by the error above
        test_stmt_err("SELECT c0 FROM t", *tx, error_code::inactive_transaction_exception);
    }
}
