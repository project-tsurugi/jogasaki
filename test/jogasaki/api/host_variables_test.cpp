...
TEST_F(host_variables_test, insert_varieties_of_types) {
    ...
    ASSERT_EQ(1, result.size());
    EXPECT_EQ(
        (mock::typed_nullable_record<kind::int4, kind::int8, kind::float8, kind::float4, kind::character>(
...
TEST_F(host_variables_test, update_temporal_types) {
    ...
    auto t12_0_0 = time_of_day{12, 0, 0};
    auto tp2000_1_1_12_0_0 = time_point{d2000_1_1, t12_0_0};
    auto d2000_2_2 = date{2000, 2, 2};
    auto t12_2_2 = time_of_day{12, 2, 2};
    auto tp2000_2_2_12_2_2 = time_point{d2000_2_2, t12_2_2};
    {
        auto ps = api::create_parameter_set();
        ps->set_date("p0", d2000_1_1);
        ps->set_time_of_day("p1", t12_0_0);
...
TEST_F(host_variables_test, insert_unknown_types) {
    ...
    execute_query("SELECT * FROM t", result);
    ASSERT_EQ(1, result.size());
    EXPECT_EQ((mock::create_nullable_record<kind::int4, kind::int4>(0, std::nullopt)), result[0]);
}
