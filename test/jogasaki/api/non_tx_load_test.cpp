...
TEST_F(non_tx_load_test, wrong_file) {
    std::vector<std::string> files{"dummy.parquet"};
    test_load(files, status::err_io_error);
}

TEST_F(non_tx_load_test, existing_file_and_missing_file) {
...
TEST_F(non_tx_load_test, cast_from_string) {
...
TEST_F(non_tx_load_test, DISABLED_third_party_file) {
    std::vector<std::string> files{
        "INT_DOUBLE.parquet"
    };
