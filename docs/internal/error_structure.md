...
- エラーカテゴリの識別のためユニークなコードが振られる(エラーコード)
  - `SQL-nnnnn` というラベルを持つ5桁の数値
  - 先頭の２桁は大分類(ルート配下のトップレベルのエラーカテゴリ)を表し、残りの3桁がトップレベル以下の小分類を表す
...
- SqlServiceException (SQL-00000)
  - SqlExecutionException (SQL-01000: SQL実行時のエラー)
    - ConstraintViolation (SQL-01001: 制約違反)
      - UniqueConstraintViolation (SQL-01002: 一意制約違反)
      - NotNullConstraintViolation (SQL-01003: Not Null 制約違反)
      - ReferentialIntegrityConstraintViolation (SQL-01004: 参照制約違反)
      - CheckConstraintViolation (SQL-01005: 検査制約違反)

    - EvaluationException (SQL-01010: SQL文の評価に関するエラー)
      - ValueException (SQL-01011: 式の値の評価に関するエラー)
    ...
    - RestrictedOperation (SQL-01020: 禁止された操作が実行された)
      ...
      - InactiveTransaction (SQL-01025: commit/abort済のトランザクションに対する操作が要求された)

    - ParameterException (SQL-01027: プレースホルダーやパラメーターに関するエラー)
      - UnresolvedPlaceholder (SQL-01028:実行要求されたステートメントが未解決のplaceholderを含む)

    - LoadFileIOException (SQL-01030: ロードファイルに関するエラー)
      - LoadFileNotFound (SQL-01031: ロードファイルが存在しない)
      - LoadFileFormatException (SQL-01032: 予期しないファイルフォーマット)
    - DumpFileIOException (SQL-01033: ダンプファイルに関するエラー)
      - DumpDirectoryInaccessible (SQL-01034: ダンプに指定されたディレクトリがアクセス可能でない)

    - SqlLimitReached (SQL-01036: 許可されたSQL操作の制限に達した)
      - TransactionExceededLimit (SQL-01037: 許同時作成可能なトランザクション数の制限を越えたためトランザクション開始に失敗した)

    - SqlRequestTimedOut (SQL-01039: SQL操作要求がタイムアウトした)

    - DataCorruption (SQL-01041: データ破損の検知)
      - SecondaryIndexCorruption (SQL-01042: セカンダリインデックスの破損) 

    - RequestFailure (SQL-01044: リクエストの処理の開始前に前提条件違反等により失敗した)
      - TransactionNotFound (SQL-01045: リクエストされたトランザクションハンドルに対応するトランザクションが存在しない or 解放済み)
      - StatementNotFound (SQL-01046: リクエストされたステートメントハンドルに対応するトランザクションが存在しない or 解放済み)

    - InternalException (SQL-01048: 内部エラーの検知)

    - UnsupportedRuntimeFeature (SQL-01050: 未サポート機能の実行)

    - BlockedByHighPriorityTransaction (SQL-01052: 高優先度のトランザクションより優先させる要求を行った)

  - CompileException (SQL-02000: コンパイル時のエラー)

    - SyntaxError (SQL-02001: 構文エラー)
     
    - AnalyzeError (SQL-02002: 解析エラー)
     
      - TypeError (SQL-02003: 型に関するエラー)
       
      - SymbolError (SQL-02004: シンボルに関するエラー)

    - UnsupportedCompilerFeature (SQL-02010: 未サポート機能/構文等のコンパイル)
  ...
  - CcException (SQL-03000: CCで直列化失敗によるエラー)
    ...
    - BlockedByConcurrentOperation (SQL-03007: 同時並行に実行された操作によってブロックされた)

### その他

- カテゴリ名はjavaの例外に対応させるためにErrorではなくExceptionという接尾辞にあうようにしている
