...
block index は basic block に対する採番だが、basic block以外も同様に扱える抽象化を検討しており、将来的に region id というものに置き換える予定である (variable-region.md 参照)。
このため、演算子のコードからは直接 block index を扱わず、抽象化した region id を使う。region_id は軽量な trivially copyable なクラスであり、block index から変換して使用する。
ただしregionの概念の導入はまだ完全ではないため、以下ではblock indexの用語も残してある。適宜region id に置き換えて読むこと。
...
- 実行時に `work_context` が `variable_table` の配列を保持し、block index 経由でアクセスする
...
- 自分のブロックで定義された変数 → 自分の `variable_table_info` が解決し、**自分の block index** を含む `value_info` を返す
- 上流ブロックで定義された変数 → 親（または祖先）に委譲し、その祖先の **block index** を含む `value_info` を返す

`value_info` には以下の情報が含まれる：

| フィールド | 説明 |
|-----------|------|
| `block_index` | 変数が実際に格納されている `variable_table` の block index |
| `value_offset` | その `variable_table` 内でのバイトオフセット |
| `nullity_offset` | nullity ビットのオフセット |
| `index` | `record_meta` 上のフィールドインデックス |

演算子は `value_info.block_index()` を使って `work_context::variables()` で得た variable_table_list から正しい `variable_table` を取得し、そこから値を読み書きする。

### buffer と variable_table の関係

buffer は変数テーブルのデータをコピーしない。下流ブロックは `value_info.block_index()` 経由で上流ブロックの `variable_table` を直接参照する。buffer 自身がすべきことは下流演算子を順番に呼び出すことのみである。

```
block 1 の variable_table: [col_a, col_b, col_c]  <- scan で定義
block 2 の variable_table: [new_col_x]             <- block 2 の project で新たに定義
block 3 の variable_table: []                      <- block 3 では新変数なし

block 2 の演算子が col_a を参照する場合:
  block_info().at(col_a) -> value_info{ block_index=1, offset=... }  (親委譲)
  work_context.variables().at(1) -> block 1 の variable_table
...
`variable_table_list` への参照とblock_index をまとめて保持する軽量なビュークラス。関係演算子内部での関数間での変数群の受け渡しに使用する。これまで `variable_table` の参照を渡していた箇所は `variables_view` を渡すように変更する。
...
例えば、 `variables_view` には `ref(std::optional<std::size_t> block_index = std::nullopt) -> accessor::record_ref` 関数があり、block_index を指定することでそれが担当する `small_record_store` の `record_ref` を得ることができる。block_index を省略した場合は自身が保持する block_index を使用する。

`variables_view` が保持する block_index は、その演算子がアクセス可能な変数の端点（basic block の末端）を表す。変数への **書き込み** 時はこの block_index をそのまま使用できるが、**読み取り** 時は `ref(block_index)` の形で祖先の block_index を明示的に指定する必要がある。

block_indexを正しく指定して呼出すのは呼出側の責任で、最大長を超えたりvariable_viewが保持するblock_indexより大きい値を指定した場合はassert_with_exceptionで失敗する。
...
`value_info` にはすでに `block_index` が含まれているため、読み取り時は `ctx.variables().ref(vinfo.block_index())` で正しい `record_ref` を取得し、書き込み時は `ctx.variables().ref()` でアクセスする。

```cpp
// 変数 v への実行時アクセスイメージ（新方式）
auto vinfo = block_info().at(v);                          // 親委譲込みで解決
auto ref = ctx.variables().ref(vinfo.block_index());      // 正しいブロックの record_ref を取得
auto value = ref.get_value<T>(vinfo.value_offset());      // 値の読み出し

// 自ブロックの変数への書き込み
auto out_ref = ctx.variables().ref();                     // 自ブロックの record_ref（block_index 省略）
