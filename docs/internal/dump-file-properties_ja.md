...
2024-01-26 kurosawa (NT)

## 本文書について

Tsurugiのダンプオプションを拡張するにあたり、メッセージ仕様を記述する
また、ファイル出力用ライブラリで設定可能な項目を調査した内容を共有する

## Arrow/Parquet共通のダンプオプション
...
* 備考
  * `max_record_count_per_file` が未指定( `0` 指定も同じ)の場合は明示的なレコード件数によるファイル分割は行わない
  * それとは独立にrecord batchの最大レコード数( `record_batch_size` や `record_batch_in_bytes` から決まる)の既定倍数(16程度)でもファイル分割が行われる
    * 現実装はarrowのみ
...
* 備考
  * 最低限の設定のみ利用可能にする
  * `record_batch_in_bytes` は直接利用できないようだが、column writer/row group writerから見積もりが取得できる(参考: https://github.com/apache/arrow/blob/3c66491846a24f17014b31a22fafdda0229f881a/cpp/examples/parquet/low_level_api/reader_writer2.cc#L88 )
    * これも使えない場合はarrow同様に平均レコードサイズから `record_batch_size` に制限をかける
...
* 備考
  ...
  * `record_batch_in_bytes` は直接利用できるか不明なので、利用できないようならテーブルスキーマから割り戻して `record_batch_size` のほうに制限を掛ける
  * 以下のように計算することを想定
    ...
    * `record_batch_size` = `0`, `max_record_batch_bytes` = `0`
      * record batch -> デフォルト値 (`64MB / 推定レコード平均サイズ`)

## 付録

### arrow/parquetライブラリで設定可能な項目
