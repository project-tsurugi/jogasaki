...
// TRUNCATE TABLEvariant of tx_remembers_drop_table
// TRUNCATE TABLE should also increments tx ref. count on the existing storage in order to avoid delete_storage while tx is active.
TEST_F(sql_lazy_delete_new_state_test, tx_remembers_truncate_table) {
    if (jogasaki::kvs::implementation_id() == "memory") {
        GTEST_SKIP() << "crash occurs only with shirakami implementation";
    }
    execute_statement("CREATE TABLE t (c0 INT PRIMARY KEY, c1 INT)");
    ...
    api::transaction_option ltx_opts{false, true, {"t"}, {}, {}, {}, true};
    auto tx_ltx = utils::create_transaction(*db_, ltx_opts);
    execute_statement("TRUNCATE TABLE t", *tx_ltx);
