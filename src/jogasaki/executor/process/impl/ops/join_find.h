 ...
 */
struct cache_align join_find_field {
    /**
     * @brief create new join_find field
     * @param type type of the found field
...
public:
    ...
    join_find(
        ...
        std::string_view key,
        std::vector<details::join_find_field> key_fields,
        std::vector<details::join_find_field> value_fields,
...
private:
    ...
    std::string key_{};
    std::vector<details::join_find_field> key_fields_{};
    std::vector<details::join_find_field> value_fields_{};
    std::unique_ptr<operator_base> downstream_{};

    void decode_fields(std::vector<details::join_find_field> const& fields, kvs::stream& stream, accessor::record_ref target, memory_resource* resource);

    std::vector<details::join_find_field> create_fields(
        yugawara::storage::index const& idx,
        sequence_view<column const> columns,
        processor_info const& info,
        block_index_type block_index,
