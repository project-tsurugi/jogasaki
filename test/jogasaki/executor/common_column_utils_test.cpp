...
using jogasaki::executor::common_column;

TEST(common_column_utils, proto_to_common_and_back_roundtrip) {
    Column src{};
    src.set_name("col1");
    src.set_atom_type(jogasaki::proto::sql::common::AtomType::INT4);
    src.set_dimension(3);
    src.set_length(42);
    ...
    auto common = jogasaki::executor::from_proto(src);

    // convert back
    auto out = jogasaki::executor::to_proto(common);
    ...
    EXPECT_TRUE(out.has_arbitrary_scale());
    EXPECT_EQ(out.nullable(), src.nullable());
    EXPECT_EQ(out.varying(), src.varying());
    EXPECT_EQ(out.description(), src.description());
}

TEST(common_column_utils, common_to_proto_and_back_roundtrip) {
    common_column c{};
    c.name_ = "col2";
    c.atom_type_ = common_column::atom_type::decimal;
    c.dimension_ = 1;
    c.length_opt_ = std::variant<std::uint32_t, bool>{static_cast<std::uint32_t>(128)};
    c.precision_opt_ = std::variant<std::uint32_t, bool>{true}; // arbitrary precision
    c.scale_opt_ = std::variant<std::uint32_t, bool>{static_cast<std::uint32_t>(2)};
    c.nullable_opt_ = false;
    c.varying_opt_ = true;
