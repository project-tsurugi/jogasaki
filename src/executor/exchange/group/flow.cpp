...
flow::flow(std::shared_ptr<shuffle_info> info, channel* ch, step* owner) : info_(std::move(info)), channel_(ch), owner_(owner) {}
...
flow::flow(std::shared_ptr<meta::record_meta> input_meta,
        ...
        flow(std::make_shared<shuffle_info>(std::move(input_meta), std::move(key_indices)), ch, owner) {}
