...
TEST_F(schema_test, variety_types) {
    ...
    EXPECT_EQ(
        (mock::typed_nullable_record<
            kind::int8, kind::character, kind::int8, kind::float8, kind::character, kind::character, kind::int8,
            kind::float8, kind::character, kind::int4, kind::float4>(
            std::tuple{
                int8_type(), character_type(true), int8_type(), float8_type(), character_type(false),
                character_type(true), int8_type(), float8_type(), character_type(false), int4_type(), float4_type()
            },
            1, text("1"), 1, 1.0, text("1"), text("1"), 1, 1.0, text("1"), 1, 1.0
        )),
        result[0]
    );
}
...
TEST_F(schema_test, DISABLED_find_double_by_decimal) {
    ...
    ASSERT_EQ(1, result.size());
    EXPECT_EQ(
        (mock::typed_nullable_record<kind::float8, kind::int4>(std::tuple{float8_type(), int4_type()}, 1.0, 1)),
        result[0]
    );
}
...
TEST_F(schema_test, nullables) {
    ...
    execute_statement("INSERT INTO TEST VALUES (2, '2', 2, 2.0, '2', 2, 2.0)");
    {
        ...
        EXPECT_EQ(
            (mock::typed_nullable_record<
                kind::int8, kind::character, kind::int8, kind::float8, kind::character, kind::int4, kind::float4>(
                std::tuple{
                    int8_type(), character_type(true), int8_type(), float8_type(), character_type(false), int4_type(),
                    float4_type()
                },
                3, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt
            )),
            result[0]
        );
    }
...
TEST_F(schema_test, ascending_keys) {
    // primary keys must be non-nullable, ascending
    auto t = std::make_shared<table>(
        "TEST",
        std::initializer_list<column>{
            column{ "C0", type::int8(), nullity{false} },
    ...
    auto i = std::make_shared<yugawara::storage::index>(
        ...
        t->simple_name(),
        std::initializer_list<index::key>{
            {t->columns()[0], sort_direction::ascendant},
            {t->columns()[1], sort_direction::ascendant},
            {t->columns()[2], sort_direction::ascendant},
            {t->columns()[3], sort_direction::ascendant},
            {t->columns()[4], sort_direction::ascendant},
            {t->columns()[5], sort_direction::ascendant},
            {t->columns()[6], sort_direction::ascendant},
        },
        std::initializer_list<index::column_ref>{
            t->columns()[7],
            t->columns()[8],
    ...
    EXPECT_EQ(
        (mock::typed_nullable_record<
            kind::int8, kind::character, kind::int8, kind::float8, kind::character, kind::int4, kind::float4,
            kind::character, kind::int8, kind::float8, kind::character, kind::int4, kind::float4>(
            std::tuple{
                int8_type(), character_type(true), int8_type(), float8_type(), character_type(false), int4_type(),
                float4_type(), character_type(true), int8_type(), float8_type(), character_type(false), int4_type(),
                float4_type()
            },
            1, text("1"), 1, 1.0, text("1"), 1, 1.0, text("1"), 1, 1.0, text("1"), 1, 1.0
        )),
        result[0]
    );
}
...
TEST_F(schema_test, default_value_with_variety_of_types) {
...

    {
        ...
        // TODO test with non-nullable record
        EXPECT_EQ(
            (mock::typed_nullable_record<kind::int8, kind::character, kind::int4, kind::float4>(
                std::tuple{int8_type(), character_type(false, 5), int4_type(), float4_type()}, 10, text("ABC  "), 2, 3.0
            )),
            result[0]
        );
    }
    {
        ...
        EXPECT_EQ(
            (mock::typed_nullable_record<kind::int8, kind::character, kind::int4, kind::float4>(
                std::tuple{int8_type(), character_type(false, 5), int4_type(), float4_type()}, 0, text("ABC  "), 20, 3.0
            )),
            result[0]
        );
    }
