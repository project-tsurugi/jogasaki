...
- DROP DDL を実行したトランザクション自身も、対象ストレージをトランザクションの使用済みストレージセットに追加する（`add_storage_ref` を呼び出す）
  - DML と同様に、DDL トランザクションがコミットまたはアボートするまで `maintenance_storage` による削除を抑制するため
  - DROP TABLE と DROP INDEX で対象ストレージは異なるが、使用済みストレージの管理上は一体として扱うものとし、プライマリストレージのみをトランザクションの使用済みストレージセットに追加する
