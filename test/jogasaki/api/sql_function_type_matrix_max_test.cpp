...
TEST_F(sql_function_type_matrix_test, DISABLED_max_tinyint) {
    ...
    test_function_with_type<kind::int4>("max(", "TINYINT", "(1),(2),(3)", 3);
}

TEST_F(sql_function_type_matrix_test, DISABLED_max_smallint) {
    db_impl()->configuration()->support_smallint(true);
    test_function_with_type<kind::int4>("max(", "SMALLINT", "(1),(2),(3)", 3);
}
...
TEST_F(sql_function_type_matrix_test, max_bigint) {
    test_function_with_type<kind::int8>("max(", "BIGINT", "(1),(2),(3)", 3);
}

TEST_F(sql_function_type_matrix_test, max_real) {
    test_function_with_type<kind::float4>("max(", "real", "(1.0e0),(2.0e0),(3.0e0)", 3.0);
}
...
TEST_F(sql_function_type_matrix_test, max_time) {
    test_function_with_type<kind::time_of_day>("max(", "TIME", "(TIME'00:00:01'),(TIME'00:00:02'),(TIME'00:00:03')", time_of_day{0, 0, 3});
}
