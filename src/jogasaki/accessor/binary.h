...
public:
     ...
     */
    [[nodiscard]] static binary copy(
        memory::paged_memory_resource& resource,
        char const* data,
        size_type size
    ) {
        return binary{std::addressof(resource), data, size};
    }

    /**
     * @brief create a new object by copying the given binary string
     * @param resource memory resource used to allocate storage for long format
     * @param str binary string data to copy
     * @return the newly created object
     */
    [[nodiscard]] static binary copy(memory::paged_memory_resource& resource, std::string_view str) {
        return binary{std::addressof(resource), str};
    }
