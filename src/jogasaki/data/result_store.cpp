...
void result_store::initialize(std::size_t count, maybe_shared_ptr<meta::record_meta> const& meta) {
...
std::size_t result_store::size() const noexcept {
    return stores_.size();
}
...
result_store::iterator const result_store::iterator::operator++(int) {
    auto ret = *this;
    ++*this;
    return ret;
}
